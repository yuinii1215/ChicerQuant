...
public class BenchmarkKLineBLImpl implements BenchmarkKLineBLService{
	...
	public KLineData dayKLineChart(String stockName) {
		...
		try{
			...
			if (oldDate.before(CalendarHelper.getPreviousDay(Calendar.getInstance()))) {
				List<BenchMark> newStocks=benchMarkDATAService.getBenchMarkAmongDate(stockName, CalendarHelper.getAfterDay(oldDate), Calendar.getInstance());
	...
	public KLineData weekKLineChart(String stockName) {
		...
		final List<KLineDataDTO> ans = oldStocks.stream().filter(ben->ben.getDateInCalendar().get(Calendar.DAY_OF_WEEK)==1).map(st -> (KLineDataDTO) st).collect(Collectors.toList());

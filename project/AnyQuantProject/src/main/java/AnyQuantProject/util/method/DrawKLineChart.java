...
public class DrawKLineChart {
    ...
    public static JFreeChart DayKLineChart(List<KLineDataDTO> dataList, List<KLineDataDTO> fiveAvgDataList, List<KLineDataDTO> tenAvgDataList, List<KLineDataDTO> thirtyAvgDataList, String id, TimeType type, String endTime) {
        ...
        double high3Value = Double.MIN_VALUE;// 设置macd的最大值
        double min3Value = Double.MAX_VALUE;// 设置macd的最低值
    ...
    public static JFreeChart DayKLineChart(List<KLineDataDTO> dataList, List<Cell> calcuList1, List<Cell> calcuList2, List<Cell> calcuList3, CalcuLineType calcuLineType, List<KLineDataDTO> fiveAvgDataList, List<KLineDataDTO> tenAvgDataList, List<KLineDataDTO> thirtyAvgDataList, String id, TimeType type, String endTime) {
        ...
        if (calcuLineType == CalcuLineType.TYPE_MACD) {
            ...
            for (int i = 0; i < calcuList3.size(); i++) {
                ...
                series3.add(new Day(date, month, year), calcuList3.get(i).y * 200);
            }
            ...
            for (int i = 0; i < calcuList3.size(); i++) {
                ...
                int month = Integer.parseInt(calcuList3.get(i).getMonth());
                int year = Integer.parseInt(calcuList3.get(i).getYear());
                series3.add(new Day(date, month, year), 0);
                System.out.println("num is:" + calcuList3.get(i).y + " date is:" + year + ":" + month + ":" + date);
            }
            timeSeriesCollection1.addSeries(series3);

            int itemCount = calcuList3.size();// 每一个序列有多少个数据项
            for (int j = 0; j < itemCount; j++) {
                if (high3Value < calcuList3.get(j).y) {// 取第i个序列中的第j个数据项的值
                    high3Value = calcuList3.get(j).y;
                }
                if (min3Value > calcuList3.get(j).y) {// 取第i个序列中的第j个数据项的值
                    min3Value = calcuList3.get(j).y;
                }
            }
        }
        // 获取K线数据的最高值和最低值
        ...
        if (calcuLineType == CalcuLineType.TYPE_MACD) {
            ...
            XYBarRenderer macdXYBarRender = new XYBarRenderer() {
                ...
                public Paint getItemPaint(int i, int j) {// 匿名内部类用来处理当日的MACD的颜色与K线图的颜色保持一致               
                    if ((j != 0) && (timeSeriesCollection1.getYValue(i, j) > timeSeriesCollection1.getYValue(i, j - 1))) {// 收盘价高于开盘价，股票上涨，选用股票上涨的颜色                                          
                        java.awt.Color red = java.awt.Color.RED;
                        return java.awt.Color.RED;
                    } else {
                        return awtColorGreen;
            ...
            plot3.setBackgroundAlpha(0.3f);
            if (calcuList1 != null) {
                if(calcuLineType == CalcuLineType.TYPE_RSI){
                KDJLineChart(plot3, calcuList1, "RSI6", 1);
                KDJLineChart(plot3, calcuList2, "RSI12", 2);
                KDJLineChart(plot3, calcuList3, "RSI24", 3);
                ...
                KDJLineChart(plot3, calcuList1, "BIAS", 1);
                KDJLineChart(plot3, calcuList2, "BIAS2", 2);
                KDJLineChart(plot3, calcuList3, "BIAS3", 3);
                }
            }
        }
...
//        combineddomainxyplot.setGap(20);// 设置两个图形区域对象之间的间隔空间             
    ...
    public static void MACDLineChart(XYPlot plot, List<Cell> macdList, String macdName, int n) {
        TimeSeries series;
        if(n==1){
           series = new TimeSeries("DIF");
        }else{
           series = new TimeSeries("DEA");
        }
        ...
        XYTitleAnnotation ta = new XYTitleAnnotation(0.007, 0.95, lt, RectangleAnchor.TOP_LEFT);
        ta.setMaxWidth(0.48);
    ...
    public static void KDJLineChart(XYPlot plot, List<Cell> kdjList, String kdjName, int n) {
        ...
        TimeSeries series = new TimeSeries(kdjName);
        ...
        if (n==1) {
            ...
            xyLineRenderer.setSeriesPaint(0, java.awt.Color.ORANGE);   //purple
        }
